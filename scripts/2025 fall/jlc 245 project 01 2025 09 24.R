...
ggplot(dc.data, aes(x = factor(months, levels = c("January",
                                                  "February",
                                                  "March",
                                                  "April",
                                                  "May",
                    ...
                    fill = months)) +
  geom_bar(stat = "count") +
  scale_fill_manual(values = c("January" = "red",
                               "February" = "blue",
                               "March"= "green",
                               "April"= "red",
                               "May"= "red",
...
# Plot with custom order for x-axis labels
ggplot(df, aes(x = factor(category, levels = c("A", "B", "C", "D")), y = value)) +
  geom_col() +
  xlab("Category") # Optionally rename the x-axis label
...
dc.data %>% ggplot() +
  ...
  scale_y_continuous(limits = c(0, 35000), breaks = c(0, 5000,10000,15000,20000,25000,30000,35000), labels = c(0,5,10,15,20,25,30,35))

dc.data %>% filter(DISTRICT != "NA") %>%
  ggplot() +
  geom_bar(aes(x = as.factor(DISTRICT), fill = TYPE), stat = "count") +
  labs(title = "Crimes by DC Police District \n2010 - 2025", x = "District", y = "Number of Crimes (Thousands)", fill = "Type of Crime") +
  theme(plot.title = element_text(hjust = 0.5, size = 14),
        axis.text.x = element_text(color = "black", size = 10),
        axis.text.y = element_text(color = "black", size = 10),
        axis.ticks.y = element_blank(),
...
dc.data %>% filter(TYPE == "Person") %>%
  ...
  labs(title = "DC xxx Crimes by Method \n200x - 202x", x = "Crime", y = "Proportion of Crime", fill = "Method") +
  theme(plot.title = element_text(hjust = 0.5, size = 14),
        axis.text.x = element_text(color = "black", size = 10),
        axis.text.y = element_text(color = "black", size = 10),
        axis.ticks.y = element_blank(),
...
### summary tables
sum.offense1 <- dc.data %>%
  group_by(OFFENSE) %>%
  summarise(COUNT = n())

sum.offense2 <- dc.data %>%
  group_by(OFFENSE) %>%
  summarise(COUNT = n()) %>%
  mutate(PCT = round(COUNT/sum(COUNT)*100,2))

sum.offense3 <- dc.data %>%
  group_by(OFFENSE, SHIFT) %>%
  summarise(COUNT = n()) %>%
  mutate(PCT = round(COUNT/sum(COUNT)*100,2))

sum.offense5 <- dc.data %>%
  group_by(months) %>%
  summarise(COUNT = n()) %>%
  mutate(PCT = round(COUNT/sum(COUNT)*100,2))

sum.offense6 <- dc.data %>%
  group_by(OFFENSE, months) %>%
  summarise(COUNT = n()) %>%
  mutate(PCT = round(COUNT/sum(COUNT)*100,2))
